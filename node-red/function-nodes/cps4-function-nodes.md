...
const base = "cps4";
const PLANT_KEY = "cps4";
const FEATURE = "navegacao";
...
const cpsId = "CPS-004";
const cpsName = "AGV_Logistico_Zeta";
const baseTopic = "cps4";
...
function buildLog(eventType, payloadData, level = "INFO", topic = null) {
  return {
    _log: {
      cpsId: payloadData?.cpsId || "CPS-004",
      cpsName: payloadData?.cpsName || "AGV_Logistico_Zeta",
      baseTopic: "cps4",
      plant: payloadData?.plant || "cps4",
      feature: payloadData?.feature || "navegacao",
      level,
      eventType,
...
if (normalizedStatus === "awaiting_replacement") {
  ...
  const cpsId = msg.payload?.cpsId || "CPS-004";
  const equipamento = msg.payload?.cpsName || "AGV_Logistico_Zeta";
  const funcionalidade = msg.payload?.feature || "navegacao";
...
`
    ...
    const outLog = buildLog(
      ...
      "WARN",
      "cps4/internal/maintenance_alert"
    );
...
if (normalizedStatus && normalizedStatus !== "awaiting_replacement") {
  ...
  const outLogReset = buildLog(
    ...
    {
      cpsId: msg.payload?.cpsId || "CPS-004",
      cpsName: msg.payload?.cpsName || "AGV_Logistico_Zeta",
      feature: msg.payload?.feature || "navegacao",
...
const msgUnplug = {
    ...
    payload: {
        cpsName: msg.payload?.cpsName || "AGV_Logistico_Zeta",
        cpsId: msg.payload?.cpsId || "CPS-004",
        baseTopic: msg.payload?.baseTopic || "cps4",
        reason: "maintenance",
        conditionType: "regra1",
...
const msgMaintenanceTimer = {
    topic: "cps4/internal/maintenance_timer",
    payload: {
...
msg.topic = "cps4/health";
msg.payload = {
    cpsId: "CPS-004",
    cpsName: "AGV_Logistico_Zeta",
    plant: p.plant || "cps4",
    feature: p.feature || "navegacao",
...
// telemetria operacional já disponível no CPS4
const live = flow.get("aas_cps4_live") || {};
...
[
  {
    ...
    "wires": [
      [
        "cps4_aas_publish_sensordata",
        "cps4_aas_debug_sensordata"
      ]
    ]
  },
  {
    "id": "cps4_aas_publish_sensordata",
    "type": "mqtt out",
    "z": "c763014928964bab",
    "name": "Publicar cps4/sensordata",
    "topic": "",
    "qos": "",
    "retain": "",
    "respTopic": "",
  ...
  },
  {
    ...
    "wires": [
      [
        "cps4_aas_publish_operationaldata",
        "cps4_aas_debug_operationaldata"
      ]
    ]
  },
  {
    "id": "cps4_aas_publish_operationaldata",
    "type": "mqtt out",
    "z": "c763014928964bab",
    "name": "Publicar cps4/operationaldata",
    "topic": "",
    "qos": "",
    "retain": "",
    "respTopic": "",
  ...
  {
    "id": "cps4_aas_debug_operationaldata",
    "type": "debug",
    "z": "c763014928964bab",
...
msg.headers = {
    ...
    "Content-Disposition": "attachment; filename=cps4-log.json"
};
...
const logEntry = {
    ts: now.toISOString(),
    epoch,
    cpsId: src.cpsId || "CPS-004",
    cpsName: src.cpsName || "AGV_Logistico_Zeta",
    baseTopic: src.baseTopic || "cps4",
...
flow.set("cps4_last_log", logEntry);
...
let currentIndex = flow.get("cps4_log_index");
if (!currentIndex) {
    currentIndex = 1;
}

const padded = String(currentIndex).padStart(3, "0");
const filename = `${baseDir}\\cps4-log-${padded}.jsonl`;
...
flow.set("cps4_log_index", currentIndex);

const padded = String(currentIndex).padStart(3, "0");
msg.filename = `${baseDir}\\cps4-log-${padded}.jsonl`;
msg.payload = msg.logLine;

return msg;
```

## Gerar telemetria operacional CPS4
```javascript
// ======================================================================
// CPS4 - Telemetria operacional com SIMULADOR ATIVO
// ======================================================================

const playEnabled = flow.get("playEnabled") === true;
const aas = flow.get("aas_cps4_model") || {};
const live = flow.get("aas_cps4_live") || {};

const baseTopic = aas?.assetInterfaces?.BaseTopic || "cps4";
...
msg.payload = {
  cpsId: aas?.shell?.cpsId || 'CPS-004',
  assetName: aas?.shell?.assetName || 'AGV_Logistico_Zeta',
  aasId: aas?.shell?.id || '',
  aasIdShort: aas?.shell?.idShort || 'CPS4_AAS',
...
## API CPS4 History
```javascript
const events = flow.get('aas_cps4_events') || [];
const live = flow.get('aas_cps4_live') || {};
...
msg.payload = {
  ...
  baseTopic: aas?.assetInterfaces?.BaseTopic || 'cps4',
...
const cpsId = aas?.shell?.cpsId || 'CPS-004';
const cpsName = aas?.shell?.assetName || 'AGV_Logistico_Zeta';
...
## Normalizar evento persistente CPS4
```javascript
let payload = msg.payload;
...
if (topic === `${baseTopic}/status`) {
    ...
    // pois eles já são persistidos no fluxo POST /api/cps4/cmd
...
```javascript
msg.filename = 'C:\\cps4-logs\\cps4-events.jsonl';
return msg;
```

## Resumo histórico CPS4
```javascript
if (msg.error) {
    msg.statusCode = 200;
    msg.headers = { 'Content-Type': 'application/json' };
...
## Preparar leitura histórico CPS4
```javascript
msg.filename = 'C:\\cps4-logs\\cps4-events.jsonl';
return msg;
```

## Montar resposta histórico CPS4
```javascript
if (msg.error) {
    msg.statusCode = 200;
    msg.headers = { 'Content-Type': 'application/json' };
...
```javascript
const logsDir = 'C:\\node-red-data\\logs';
const pattern = 'cps4-log-*.jsonl';

msg.command = `powershell.exe -NoProfile -Command "if (Get-ChildItem -Path '${logsDir}\\${pattern}' -ErrorAction SilentlyContinue) { Get-ChildItem -Path '${logsDir}\\${pattern}' | Sort-Object Name | ForEach-Object { Get-Content $_.FullName } } elseif (Test-Path '${logsDir}\\cps4-log.jsonl') { Get-Content '${logsDir}\\cps4-log.jsonl' } else { Write-Output '' }"`;
return msg;
```
...
const cpsId = firstEvent?.cpsId || 'CPS-004';
const cpsName = firstEvent?.cpsName || 'AGV_Logistico_Zeta';
const baseTopic = firstEvent?.baseTopic || 'cps4';
...
const pdfDir = 'C:\\cps4-logs';
const pdfPath = `${pdfDir}\\cps4-scientific-report-${ts}.pdf`;
const url = 'http://127.0.0.1:1880/api/cps4/report/scientific';
...
msg.headers = {
    ...
    'Content-Disposition': 'attachment; filename="cps4-scientific-report.pdf"'
};
return msg;
```
...
msg.headers = {
    'Content-Type': 'application/json; charset=utf-8',
    'Content-Disposition': 'attachment; filename="cps4-events.jsonl"'
};
return msg;
```

## Montar HTML estático CPS4
```javascript
msg.headers = { 'Content-Type': 'text/html; charset=utf-8' };
msg.payload = `<!DOCTYPE html>
<html lang="pt-BR">
...
<body>
<div class="header">
  <h1>CPS-7 · AGV_Logistico_Zeta</h1>
</div>
<div class="container">
  <img src="/img/50464_Stan.png" onerror="this.style.display='none'" />

  <div class="grid">
    <div class="card"><h3>CPS ID</h3><b>CPS-004</b></div>
  ...
  <div class="buttons">
    <a class="btn json" href="/api/cps4/log/json" target="_blank">JSON</a>
    <a class="btn pdf" href="/api/cps4/report/scientific/pdf" target="_blank">PDF</a>
  </div>
...
async function loadSummary() {
  try {
    const res = await fetch('/api/cps4/report/scientific');
...
async function loadTimeline() {
  try {
    const res = await fetch('/api/cps4/timeline/recent');
...
```javascript
msg.filename = 'C:\\cps4-logs\\cps4-datasheet.pdf';
return msg;
```
...
msg.headers = {
    'Content-Type': 'application/pdf',
    'Content-Disposition': 'inline; filename="cps4-datasheet.pdf"'
};
return msg;
```
...
const cpsId = fullPayload.cpsId || "cps4";
...
```javascript
// ============================================================
// CPS4 - Local Time-Series Features
...
// Histórico
// -----------------------------
let history = flow.get("cps4_ts_history") || [];
history.push(point);
history = lastN(history, HISTORY_SIZE);
flow.set("cps4_ts_history", history);
...
// -----------------------------
flow.set("cps4_last_timeSeriesFeatures", features);
...
if (!ready) {
    ...
    flow.set("cps4_last_learning", learning);
...
if (
    ...
    recommendation = "Prioritize intervention and inspect CPS4 immediately.";
}
...
## Build CPS4 Payload for ACSM
```javascript
// ============================================================
// CPS4 - Build Payload for ACSM
// Entrada esperada:
...
// --------------------------------------------------
const cpsId = p.cpsId || "CPS-004";
const cpsName = p.cpsName || "AGV_Logistico_Zeta";
const baseTopic = "cps4";
...
const acsmPayload = {
    ...
    source: {
        ...
        generatedBy: "Build CPS4 Payload for ACSM"
    }
};
...
// --------------------------------------------------
flow.set("cps4_last_acsm_payload", acsmPayload);

let acsmPayloadHistory = flow.get("cps4_acsm_payload_history") || [];
...
flow.set("cps4_acsm_payload_history", acsmPayloadHistory);
