...
const base = "cps2";
const PLANT_KEY = "cps2";
const FEATURE = "desbaste";
...
const cpsId = "CPS-002";
const cpsName = "CNC_5Eixos_Evo";
const baseTopic = "cps2";
...
function buildLog(eventType, payloadData, level = "INFO", topic = null) {
  return {
    _log: {
      cpsId: payloadData?.cpsId || "CPS-002",
      cpsName: payloadData?.cpsName || "CNC_5Eixos_Evo",
      baseTopic: "cps2",
      plant: payloadData?.plant || "cps2",
      feature: payloadData?.feature || "desbaste",
      level,
      eventType,
...
if (normalizedStatus === "awaiting_replacement") {
  ...
  const equipamento = msg.payload?.cpsName || "CNC_5Eixos_Evo";
  const funcionalidade = msg.payload?.feature || "desbaste";
...
`
    ...
    const outLog = buildLog(
      ...
      "WARN",
      "cps2/internal/maintenance_alert"
    );
...
if (normalizedStatus && normalizedStatus !== "awaiting_replacement") {
  ...
  const outLogReset = buildLog(
    ...
    {
      cpsId: msg.payload?.cpsId || "CPS-002",
      cpsName: msg.payload?.cpsName || "CNC_5Eixos_Evo",
      feature: msg.payload?.feature || "desbaste",
...
const msgUnplug = {
    ...
    payload: {
        cpsName: msg.payload?.cpsName || "CNC_5Eixos_Evo",
        cpsId: msg.payload?.cpsId || "CPS-002",
        baseTopic: msg.payload?.baseTopic || "cps2",
        reason: "maintenance",
        conditionType: "regra1",
...
const msgMaintenanceTimer = {
    topic: "cps2/internal/maintenance_timer",
    payload: {
...
msg.topic = "cps2/health";
msg.payload = {
    cpsId: "CPS-002",
    cpsName: "CNC_5Eixos_Evo",
    plant: p.plant || "cps2",
    feature: p.feature || "desbaste",
...
// telemetria operacional já disponível no CPS2
const live = flow.get("aas_cps2_live") || {};
...
[
  {
    ...
    "wires": [
      [
        "cps2_aas_publish_sensordata",
        "cps2_aas_debug_sensordata"
      ]
    ]
  },
  {
    "id": "cps2_aas_publish_sensordata",
    "type": "mqtt out",
    "z": "c763014928964bab",
    "name": "Publicar cps2/sensordata",
    "topic": "",
    "qos": "",
    "retain": "",
    "respTopic": "",
  ...
  },
  {
    ...
    "wires": [
      [
        "cps2_aas_publish_operationaldata",
        "cps2_aas_debug_operationaldata"
      ]
    ]
  },
  {
    "id": "cps2_aas_publish_operationaldata",
    "type": "mqtt out",
    "z": "c763014928964bab",
    "name": "Publicar cps2/operationaldata",
    "topic": "",
    "qos": "",
    "retain": "",
    "respTopic": "",
  ...
  {
    "id": "cps2_aas_debug_operationaldata",
    "type": "debug",
    "z": "c763014928964bab",
...
msg.headers = {
    ...
    "Content-Disposition": "attachment; filename=cps2-log.json"
};
...
const logEntry = {
    ts: now.toISOString(),
    epoch,
    cpsId: src.cpsId || "CPS-002",
    cpsName: src.cpsName || "CNC_5Eixos_Evo",
    baseTopic: src.baseTopic || "cps2",
...
flow.set("cps2_last_log", logEntry);
...
let currentIndex = flow.get("cps2_log_index");
if (!currentIndex) {
    currentIndex = 1;
}

const padded = String(currentIndex).padStart(3, "0");
const filename = `${baseDir}\\cps2-log-${padded}.jsonl`;
...
flow.set("cps2_log_index", currentIndex);

const padded = String(currentIndex).padStart(3, "0");
msg.filename = `${baseDir}\\cps2-log-${padded}.jsonl`;
...
```javascript
// ======================================================================
// CPS2 - Telemetria operacional com SIMULADOR ATIVO
// ======================================================================

const playEnabled = flow.get("playEnabled") === true;
const aas = flow.get("aas_cps2_model") || {};
const live = flow.get("aas_cps2_live") || {};

const baseTopic = aas?.assetInterfaces?.BaseTopic || "cps2";
...
msg.payload = {
  cpsId: aas?.shell?.cpsId || 'CPS-002',
  assetName: aas?.shell?.assetName || 'CNC_5Eixos_Evo',
  aasId: aas?.shell?.id || '',
  aasIdShort: aas?.shell?.idShort || 'CPS2_AAS',
...
## API CPS2 History
```javascript
const events = flow.get('aas_cps2_events') || [];
const live = flow.get('aas_cps2_live') || {};
...
msg.payload = {
  ...
  baseTopic: aas?.assetInterfaces?.BaseTopic || 'cps2',
...
const cpsId = aas?.shell?.cpsId || 'CPS-002';
const cpsName = aas?.shell?.assetName || 'CNC_5Eixos_Evo';
...
if (topic === `${baseTopic}/status`) {
    ...
    // pois eles já são persistidos no fluxo POST /api/cps2/cmd
...
```javascript
msg.filename = 'C:\\cps2-logs\\cps2-events.jsonl';
return msg;
```
...
const cpsId = firstEvent?.cpsId || 'CPS-002';
const cpsName = firstEvent?.cpsName || 'CNC_5Eixos_Evo';
const baseTopic = firstEvent?.baseTopic || 'cps2';
...
const pdfDir = 'C:\\cps2-logs';
const pdfPath = `${pdfDir}\\cps2-scientific-report-${ts}.pdf`;
const url = 'http://127.0.0.1:1880/api/cps2/report/scientific';
...
msg.headers = {
    ...
    'Content-Disposition': 'attachment; filename="cps2-scientific-report.pdf"'
};
return msg;
```
...
msg.headers = {
    'Content-Type': 'application/json; charset=utf-8',
    'Content-Disposition': 'attachment; filename="cps2-events.jsonl"'
};
return msg;
```
...
<body>
<div class="header">
  <h1>CPS-7 · CNC_5Eixos_Evo</h1>
</div>
<div class="container">
  <img src="/img/50464_Stan.png" onerror="this.style.display='none'" />

  <div class="grid">
    <div class="card"><h3>CPS ID</h3><b>CPS-002</b></div>
  ...
  <div class="buttons">
    <a class="btn json" href="/api/cps2/log/json" target="_blank">JSON</a>
    <a class="btn pdf" href="/api/cps2/report/scientific/pdf" target="_blank">PDF</a>
  </div>
...
async function loadSummary() {
  try {
    const res = await fetch('/api/cps2/report/scientific');
...
async function loadTimeline() {
  try {
    const res = await fetch('/api/cps2/timeline/recent');
...
```javascript
msg.filename = 'C:\\cps2-logs\\cps2-datasheet.pdf';
return msg;
```
...
msg.headers = {
    'Content-Type': 'application/pdf',
    'Content-Disposition': 'inline; filename="cps2-datasheet.pdf"'
};
return msg;
```
...
const cpsId = fullPayload.cpsId || "cps2";
...
```javascript
// ============================================================
// CPS2 - Local Time-Series Features
...
// Histórico
// -----------------------------
let history = flow.get("cps2_ts_history") || [];
history.push(point);
history = lastN(history, HISTORY_SIZE);
flow.set("cps2_ts_history", history);
...
// -----------------------------
flow.set("cps2_last_timeSeriesFeatures", features);
...
if (!ready) {
    ...
    flow.set("cps2_last_learning", learning);
...
if (
    ...
    recommendation = "Prioritize intervention and inspect CPS2 immediately.";
}
...
// --------------------------------------------------
const cpsId = p.cpsId || "CPS-002";
const cpsName = p.cpsName || "CNC_5Eixos_Evo";
const baseTopic = "cps2";
...
const acsmPayload = {
    ...
    source: {
        ...
        generatedBy: "Build CPS2 Payload for ACSM"
    }
};
...
// --------------------------------------------------
flow.set("cps2_last_acsm_payload", acsmPayload);

let acsmPayloadHistory = flow.get("cps2_acsm_payload_history") || [];
...
flow.set("cps2_acsm_payload_history", acsmPayloadHistory);
