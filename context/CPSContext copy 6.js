...
const ACSM_OEE_WILDCARD_TOPIC = 'acsm/+/oee';
const ACSM_LEARNING_WILDCARD_TOPIC = 'acsm/+/learning';
...
export const CPSProvider = ({ children }) => {
  ...
  const handleAnalyticsMessage = useCallback((topic, rawPayload) => {
    try {
      ...
      const cpsId = parsed?.cpsId || 'UNKNOWN';
      if (!cpsId) return;
      ...
      console.error('Erro ao processar mensagem analytics MQTT:', err, topic, rawPayload);
    }
  }, []);
    ...
    const start = async () => {
      try {
        ...
        client.on('connect', () => {
          ...
          const analyticsSubs = [
            ...topicVariants(ACSM_OEE_WILDCARD_TOPIC),
            ...topicVariants(ACSM_LEARNING_WILDCARD_TOPIC),
          ];
            ...
            setLog((prev) => [
              ...prev,
              {
                ...
                message: err
                  ...
                  : `[MQTT] Subscribed analytics: ${analyticsSubs.join(', ')}`,
              },
            ]);
          });
        });
        ...
        client.on('message', (topic, message) => {
          ...
          const isAnalyticsTopic =
            normIncoming.startsWith('acsm/') &&
            (normIncoming.endsWith('/oee') || normIncoming.endsWith('/learning'));
