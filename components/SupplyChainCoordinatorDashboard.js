    ...
    <div style={{ display: 'grid', gap: 18 }}>
      ...
      >
        ...
        >
          <div>
            ...
            >
              Executive panel for reviewing ACSM knowledge and the interorganizational decision generated by the supply chain coordinator.
            </div>
      ...
      >
        <MetricCard
          ...
          helper="Consolidated Global OEE received from the ACSM."
        />
        <MetricCard
          label="Critical CPS"
          value={decision?.criticalCPS || '—'}
          helper="Most critical asset identified by the coordinator."
        />
        <MetricCard
          label="Confidence"
          value={toNumber(decision?.confidence) !== null ? decision.confidence.toFixed(2) : '—'}
          helper="Confidence associated with the interorganizational decision."
        />
        <MetricCard
          label="High Priority Actions"
          value={String(decision?.summary?.highPriorityCount ?? prioritizedActions.length ?? 0)}
          helper="Number of high-priority actions."
        />
      </div>
