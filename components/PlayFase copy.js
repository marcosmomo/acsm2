...
const openAnalyticsPage = (cps) => {
  const rawId = cps?.id || 'cps7';
  const cleanId = rawId.toLowerCase().replace(/[^a-z0-9]/g, '').replace(/([a-z]+)0+/, '$1');
        ...
        <ul className="cps-list-play">
          {addedCPS.length > 0 ? (
            addedCPS.map((cps) => {
              ...
              const isStopped = String(cps.status || '').toLowerCase() === 'parado';
              const isRunning = String(cps.status || '').toLowerCase() === 'rodando';
